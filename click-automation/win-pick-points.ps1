...
# Abrir Chrome com URL se fornecido
if (-not [string]::IsNullOrWhiteSpace($OpenUrl)) {
  Write-Host "Abrindo Chrome com URL: $OpenUrl"

  # Tentar encontrar o Chrome em varios locais
  $chromePaths = @(
  ...
  if ($chromePath) {
    ...
    Start-Process -FilePath $chromePath -ArgumentList $OpenUrl
    Start-Sleep -Milliseconds 3000
  } else {
    # Tentar abrir usando o comando 'start' do Windows
    Write-Host "Tentando abrir Chrome usando comando start..."
    try {
      Start-Process "chrome.exe" -ArgumentList $OpenUrl -ErrorAction SilentlyContinue
      Start-Sleep -Milliseconds 3000
    } catch {
      # Tentar abrir URL direta (Windows abre com navegador padrao)
